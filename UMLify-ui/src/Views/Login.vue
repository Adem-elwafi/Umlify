  ...
  <div class="min-h-screen w-full flex items-center justify-center bg-zinc-50 bg-[radial-gradient(#e4e4e7_1px,transparent_1px)] [background-size:16px_16px] p-4">
    <div class="w-full max-w-sm bg-white border border-zinc-200 shadow-xl shadow-zinc-200/30 rounded-2xl p-6 md:p-8 flex flex-col">
      <div class="mb-8">
        ...
        <h2 class="text-xl font-semibold tracking-tight text-zinc-900">Welcome Back</h2>
        <p class="text-xs text-zinc-400 font-medium tracking-normal mt-1">Log in to access your engineering canvases</p>
      </div>

      <div v-if="authStore.apiErrorMessage" class="mb-6 p-3 bg-red-50 border border-red-200 rounded-lg text-[11px] font-medium text-red-600 text-center">
        {{ authStore.apiErrorMessage }}
      </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-4">
        ...
        <button 
          type="submit" 
          :disabled="isLoading"
          class="w-full py-2 bg-zinc-900 hover:bg-zinc-800 text-white font-semibold text-xs rounded-lg shadow-sm transition-all active:scale-98 cursor-pointer flex items-center justify-center"
        >
      ...
      <div class="mt-8 text-center">
        <p class="text-xs text-zinc-400 font-medium">
          ...
          <router-link to="/signup" class="text-blue-600 hover:underline cursor-pointer ml-1">Create an account</router-link>
        </p>
      </div>
    </div>
  </div>
